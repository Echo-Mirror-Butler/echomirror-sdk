...
//!
//! ```sh
//! REDIS_URL=redis://127.0.0.1:6379 cargo test -p echomirror-sync --features redis
//! ```
#![cfg(feature = "redis")]

use echomirror_sync::{CursorStore, RedisCursorStore, SyncCursor};
...
async fn connect_with_custom_ttl() {
    let Some(url) = redis_url() else { return };
    let _store = RedisCursorStore::connect_with_ttl(&url, 3600).await.unwrap();
}
...
async fn multiple_accounts_independent() {
    ...
    let account1 = format!("GTEST1_{}", chrono::Utc::now().timestamp_nanos_opt().unwrap());
    let account2 = format!("GTEST2_{}", chrono::Utc::now().timestamp_nanos_opt().unwrap());
...
async fn ttl_refresh_on_save() {
    ...
    let loaded = store.load(&account).await.unwrap().expect("cursor still present");
